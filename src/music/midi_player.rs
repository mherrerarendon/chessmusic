...
impl MidiPlayer {
    ...
    pub fn play_note(&mut self, note: u8) {
        ...
        let ret = self.conn_out.send(&[NOTE_OFF_MSG, note, VELOCITY]);
    }

    fn get_conn_out() -> MidiOutputConnection {
        let midi_out = match MidiOutput::new("My Test Output") {
            Ok(midi_out) => midi_out,
            Err(the_error) => panic!("Failed to create new midi output")
        };
        ...
        println!("\nOpening connection");
        let mut conn_out = midi_out.connect(out_port, "doesn't seem to matter").unwrap();
