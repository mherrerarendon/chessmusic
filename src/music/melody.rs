...
impl Melody {
    pub fn new(move_history_with_captures: &[(Cell, bool)], initial_note: Note) -> Melody {
        Melody {
            notes: Melody::compose_with_move_take_history(move_history_with_captures, initial_note)
        }
    }

    fn compose_with_move_take_history(move_history_with_captures: &[(Cell, bool)], initial_note: Note) -> Vec<Note> {
...
mod tests {
    ...
    fn test_compose_with_cell_history_vertical() {
        ...
        let cell_history = &[(Cell::new("a2"), false), (Cell::new("a3"), false), (Cell::new("a4"), false)];
        let melody = Melody::compose_with_move_take_history(cell_history, starting_note);
    ...
    fn test_compose_with_cell_history_horizontal() {
        let starting_note = Note::new(50);
        let cell_history = &[(Cell::new("a2"), false), (Cell::new("b2"), false), (Cell::new("c2"), false)];
        let melody = Melody::compose_with_move_take_history(cell_history, starting_note);
        assert_eq!(melody.len(), 3);
        assert_eq!(melody[0].as_midi(), 50);
    ...
    fn test_compose_with_cell_history_diagonal() {
        let starting_note = Note::new(50);
        let cell_history = &[(Cell::new("a2"), false), (Cell::new("b3"), false), (Cell::new("c4"), false)];
        let melody = Melody::compose_with_move_take_history(cell_history, starting_note);
        assert_eq!(melody.len(), 3);
        assert_eq!(melody[0].as_midi(), 50);
